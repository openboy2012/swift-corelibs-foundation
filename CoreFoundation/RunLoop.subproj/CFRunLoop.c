...
static CFRunLoopModeRef __CFRunLoopFindMode(CFRunLoopRef rl, CFStringRef modeName, Boolean create) {
    ...
    if (NULL != rlm) {
        __CFRunLoopModeLock(rlm); //返回有值时都加锁处理，外部使用时都需要先解锁才能多线程可用。
        return rlm;
    }
    if (!create) {
        return NULL;
...
static void __CFRunLoopDoObservers(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopActivity activity) {	/* DOES CALLOUT */
    ...
    __CFRunLoopModeUnlock(rlm); //pthread_mutex在不加锁的情况下进行unlock一次没有任何warning，但NSRecursiveLock在不加锁的情况下会报warning, CFRunLoop.c实现文件中有多次出现了这种情况。
    __CFRunLoopUnlock(rl);
...
#define TIMEOUT_INFINITY (~(mach_msg_timeout_t)0) //按位取反，unsign int表示取最大值4294967295
